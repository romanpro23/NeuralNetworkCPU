...
public class ImageNet500Loader3D extends DataLoader3D {
    ...
    public ImageNet500Loader3D() {
        this(new TransformData.Sigmoid());
    }

    public ImageNet500Loader3D(TransformData transformData) {
        this.transformData = transformData;

        test = new ArrayList<>();
        train = new ArrayList<>();
        labels = new ArrayList<>();
        sizeLabelsV = new int[500];
        sizeLabelsT = new int[500];
        nT = new int[500];
        nV = new int[500];

        try {
            Scanner reader = new Scanner(new File("D:\\datasets\\ImageNet\\labels500.txt"));
            int i = 0;
    ...
    public ImageNet500Loader3D setTransformData(TransformData transformData){
        this.transformData = transformData;
    ...
    protected void reloadTestData(){
        test.removeAll(test);
        for (int i = 0; i < 500; i++) {
            NNVector output = new NNVector(500);
            output.set(i, 1);
            for (int j = 0; j < 2; j++) {
    ...
    public ImageNet500Loader3D useCrop(){
        this.useCrop = true;

        return this;
    }

    public ImageNet500Loader3D useReverse(){
    ...
    public static String[] getLabelVal(int... val){
        Scanner readerVal = new Scanner(new File("D:\\datasets\\ImageNet\\label500_val.txt"));
        ArrayList<String> labelsVal = new ArrayList<>(500);
