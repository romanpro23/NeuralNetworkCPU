...
public class SNConvolutionTransposeLayer extends NeuralLayer3D {
    //trainable parts
    ...
    private float spectralNorm() {
        NNMatrix weightM = new NNMatrix(depth, v.size(), weight.getData(),  weight.getSdata());
    ...
    public void generateOutput(NNArray[] inputs) {
        ...
        for (int t = 0; t < input.length; t++) {
            ...
            executor.execute(() -> {
                ...
                output[i].transposeConvolution(input[i], weight, stride, paddingY, paddingX);
                if (useBias) {
