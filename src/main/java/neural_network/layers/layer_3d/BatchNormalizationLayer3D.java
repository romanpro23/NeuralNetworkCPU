...
public class BatchNormalizationLayer3D extends NeuralLayer3D {
    //trainable parts
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            for (int j = 0; j < size; j++) {
                for (int k = 0; k < depth; k++, index++) {
                    output[i].getData()[index] = (input[i].getData()[index] - mean.get(k)) / varSqrt[k];
                    output[i].getData()[index] = output[i].getData()[index] * gamma.get(k) + betta.get(k);
                }
            }
        }
    }
