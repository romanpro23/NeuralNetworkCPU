...
public class LSTMBahdAttentionLayer extends BahdanauAttentionLayer {
    ...
    public LSTMBahdAttentionLayer(int countNeuron, int sizeAttention) {
        this(countNeuron, sizeAttention, 0);
    }

    public LSTMBahdAttentionLayer(LSTMBahdAttentionLayer layer) {
        this(layer.countNeuron, layer.sizeAttention, layer.recurrentDropout, layer.returnSequences);
        this.copy(layer);
    }

    public LSTMBahdAttentionLayer(int countNeuron, int sizeAttention, double recurrentDropout) {
    ...
    public LSTMBahdAttentionLayer(int countNeuron, int sizeAttention, double recurrentDropout, boolean returnSequences) {
        this(countNeuron, sizeAttention, recurrentDropout);
        setReturnSequences(returnSequences);
    }

    public LSTMBahdAttentionLayer setReturnSequences(boolean returnSequences) {
    ...
    public int info() {
        ...
        System.out.println("Bahd LSTM\t|  " + width + ",\t" + depth + "\t\t|  " + outWidth + ",\t" + countNeuron + "\t\t|\t" + countParam);
        return countParam;
    }

    @Override
    public void save(FileWriter writer) throws IOException {
        writer.write("LSTM bahdanau attention layer\n");
    ...
    private void generateOutput(int i, NNVector[] states) {
        ...
        for (int t = 0, tOut = 0; t < input[i].getRow(); t++) {
            ...
            gateFInput[i][t].addMul(inputVector[i][t], weightInput[0]);
            gateIInput[i][t].addMul(inputVector[i][t], weightInput[1]);
            gateOInput[i][t].addMul(inputVector[i][t], weightInput[2]);
            gateCInput[i][t].addMul(inputVector[i][t], weightInput[3]);
            if (hiddenS_t != null) {
    ...
    public LSTMBahdAttentionLayer setRegularization(Regularization regularization) {
        this.regularization = regularization;

        return this;
    }

    public LSTMBahdAttentionLayer setInitializer(Initializer initializer) {
    ...
    public LSTMBahdAttentionLayer setInitializerInput(Initializer initializer) {
        this.initializerInput = initializer;

        return this;
    }

    public LSTMBahdAttentionLayer setInitializerHidden(Initializer initializer) {
        this.initializerHidden = initializer;

        return this;
    }

    public LSTMBahdAttentionLayer setTrainable(boolean trainable) {
        this.trainable = trainable;

        return this;
    }

    public static LSTMBahdAttentionLayer read(Scanner scanner) {
        LSTMBahdAttentionLayer recurrentLayer = new LSTMBahdAttentionLayer(Integer.parseInt(scanner.nextLine()),
